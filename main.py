...
def fetch_attachment(uid):
    files = []
    client = WebClient(
        token="xoxp-535944217620-535998288195-1006436390965-fb8f9cd8f02288c6333b3ca17893bd50")
...
def reply(from_email, to, subject, content):
    print(from_email, to, subject, content)
...
def post_unread():
    ...
    if unread_messages:
        for msg in unread_messages:
            ...
            if msg["attachments"]:
                attachments_ = ' '.join(
                    [f"<{ach}|link>" for ach in msg["attachments"]])
            else:
                attachments_ = "None"
